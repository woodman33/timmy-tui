...
function setup() {
  ...
  if (nim) {
    const nimAdd = spawnSync(JCODE, ['provider', 'add', 'node-a', '--base-url', nim, '--model', flag('--nim-model', 'nvidia/llama-3.3-nemotron-super-49b-v1'), '--api-key-env', 'NVIDIA_NIM_API_KEY'], { cwd: HOME, env: childEnv(), encoding: 'utf8' });
    profiles.push({ name: 'node-a', base_url: nim, ok: nimAdd.status === 0, note: 'nvidia-nim on spark2; proven only when the NIM server answers' });
  } else {
    profiles.push({ name: 'node-a', base_url: null, ok: false, note: 'spark2 address not in the node overlay yet (engine-room WAITING-ON-WILL); profile deferred' });
  }
