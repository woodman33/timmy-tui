...
export interface WorkflowRequestRecord extends WorkflowHttpRecord {
  turn: number; phase: 'tools' | 'final'; promptTokens: number | null; completionTokens: number | null;
...
export interface WorkflowEpisodeResult {
  ...
  error: string | null; protocolErrors: string[]; turns: number; toolRounds: number; finalRequests: number;
...
export async function runWorkflowModelEpisode(input: {
  ...
  const result: WorkflowEpisodeResult = { schema: 'timmy.workflow-model-episode/1', model: manifest.exactTag, modelDigest: manifest.digest, manifestSha256, toolsSha256, contextWindow, budget,
    ...
    turns: 0, toolRounds: 0, finalRequests: 0, attemptedCalls: 0, executedCalls: 0, elapsedMs: 0, requestDurationMs: 0, toolPhaseEnd: null,
  ...
  async function chat(phase: 'tools' | 'final', until: number) {
    ...
    const body = freeze(copy({ model: manifest.exactTag, stream: false, think: false, truncate: false, shift: false, keep_alive: '2m',
      messages, ...(phase === 'tools' ? { tools } : { format: finalFormat }),
      options: { temperature: 0, seed: 42, num_ctx: contextWindow, num_predict: phase === 'tools' ? budget.toolPredict : budget.finalPredict } })) as unknown as LabRecord;
    ...
    result.requests.push(entry); result.turns++; if (phase === 'tools') result.toolRounds++; else result.finalRequests++;
    try {
      ...
      if (entry.completionTokens !== null && entry.completionTokens > (phase === 'tools' ? budget.toolPredict : budget.finalPredict)) throw new Error('Reported generation exceeds frozen token budget.');
  ...
  try {
    ...
    assertFrozen();
    messages.push({ role: 'user', content: 'FINAL: Tool use is finished. Return exactly one JSON object matching the original task answer schema, using only available observations and actual evidence IDs. Preserve unknowns; never substitute empty for unknown. Do not invent measurements or citations. No Markdown, prose, extra tool calls or further plans. If required evidence is absent, report insufficient_evidence according to the task schema. This is your one reserved final response.' });
