...
function snapshotImplementation(directory: string) {
  const paths = ['workflow-benchmark.ts', 'workflow-model-adapter.ts', 'workflow-isolation.ts', 'lab-challenges.ts', 'lab-model-adapter.ts', 'lab-independent-verifier.ts'];
...
async function runWorkflowEvaluation(directory: string, modelTag: string, progress: (message: string) => void, requestedCase?: string) {
  ...
  const protocol = { schema: 'timmy.workflow-canary.protocol/1', mode: requestedCase ? 'explicit-single-case' : 'gated-canary', model: manifest, sourceHashes, selected, budgets: WORKFLOW_BUDGETS,
    maximumModels: 1, maximumEpisodes: selected.length, concurrency: 1, stop: 'First protocol, integrity, timeout, or task failure. Diagnose before expanding.',
  ...
  for (const id of selected) {
    ...
    const episode = await runWorkflowModelEpisode({ manifest, initialMessages: workflowMessages(item),
      ...
      dispatch: (name: string, args: LabRecord) => clone(bound.dispatch(name, args)), budget: WORKFLOW_BUDGETS[item.tier], finalFormat: workflowFinalSchema(item) });
