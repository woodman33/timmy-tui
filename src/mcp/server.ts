...
const TOOLS = [
  ...
  { name: 'timmy_promo_apply', description: 'Apply a fused beats delta to the promo comp: replaces claim/sub/evidence text per beat id. from = source comp (default timmy-promo-v8); output is the next version (v9 → v10 etc).', inputSchema: { type: 'object', properties: { beats: { type: 'array', description: 'per-beat text deltas', items: { type: 'object', properties: { id: { type: 'string' }, claim: { type: 'string' }, sub: { type: 'string' }, evidence: { type: 'string' } }, required: ['id'] } }, from: { type: 'string', description: 'source comp dir, default timmy-promo-v8' } }, required: ['beats'] } },
