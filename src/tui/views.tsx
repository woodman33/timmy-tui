...
export function ViewStage({ view, paneFocus, agent, setInspector, setModalInput, inputLocked }: ViewStageProps) {
  ...
  if (view === 0) {
    const commandWidth = Math.max(24, Math.floor(width * 0.6) - 5);
    const commandHeight = Math.max(6, height - 3);
      ...
      <Box flexDirection="row" flexGrow={1}>
        <Box flexGrow={3} flexDirection="column" paddingRight={1}>
          ...
          >
            ...
            <CommandView agent={agent} width={commandWidth} height={commandHeight} />
          </Box>
        </Box>
